...
// WiFi: Credenciais
const char* ssid = "ANDROMEDA"; // <- set your SSID here
const char* password = "22602260"; // <- Set your WIFI password here
...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    } // <- Adicionado fechamento do if do ThingSpeak

    // JSON consolidado
    String payload = "{\"temperatura\":";
    payload += String(temp, 1);
    payload += ",\"umidade\":";
    payload += String(umid, 1);
    payload += ",\"indice_solar\":";
    payload += String(insolacaoPercent, 0);
    payload += ",\"chuva\":\"";
    payload += (chovendo ? "Chovendo" : "Sem chuva");
    payload += "\",\"alerta\":\"";
    payload += alerta;
    payload += "\",\"timestamp\":\"";
    payload += timestamp;
