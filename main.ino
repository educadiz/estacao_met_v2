/* 
  ...
  Código desenvolvido por Eduardo Cadiz para uso no projeto da Estação Meteorológica
  Este código engloba publicações de dados via MQTT, Blynk e Thingspeak.
  Observe abaixo os parâmetros e linhas de comentários.
  Hardware usado:
  - Microcontrolador ESP32
  - Sensor de temperatura e umidade DHT11
  - Display modelo GMT-020-02 240x320 colorido
  - LDR
  - 2 x resistores 220r ohm
  - 1 x resistor 10k ohm
  - Sensor de chuva Raindrops
  - Buzzer ativo
*/

// Configuração Blynk:
// Caso não vá usar a plataforma Blynk para visualização dos dados, comente as linhas abaixo.
#define BLYNK_TEMPLATE_ID "TMPL2EzjqzR7P"                    //  ID do Template no Blynk
#define BLYNK_TEMPLATE_NAME "EMC01"                          //  Nome do Template no Blynk
#define BLYNK_AUTH_TOKEN "y0YIz5He8zrIW1BF-0eHwQAN7VdxJkAK"  //  Token de autenticação no Blynk
...
// Modelo do display usado no projeto: Display GMT020-02-7P v1.3 (TFT 2" ST7789) 240x320 pixels
...
// Abaixo insira os dados de sua rede Wifi (principal e backup)
...
// Dados do MQTT:
// Aqui abaixo você pode informar qual Broker MQTT vai usar bem como o Client ID e os tópicos
// todos os tópicos estão marcados apenas como exemplo. Lembre-se de usar os mesmos tópicos
// ao subscrever em seu app cliente IOT.
const char* mqtt_server = "broker.hivemq.com";
const int mqtt_port = 1883;
const char* clientID = "Exemplo_ID01";
const char* topicChuva = "exempploID01/chuva";
const char* topicTemp = "exemploID01/temp";
const char* topicUmid = "exemploID01/umid";
const char* topicSolar = "exemploID01/solar";
const char* topicAlerta = "exemploID01/alerta";
const char* topicDados = "exemploID01/dados";

// Configuração API ThingSpeak:
// Abaixo estão os dados para configuração na plataforma Thingspeak
// Observe os parâmetros usados em sua conta para caso queira usar o Thingspeak
// como dashbard.
const char* ts_server = "api.thingspeak.com";     // Dados do Thingspeak Server
const char* writeAPIKey = "2PI8MD4NVFEY9XSZ";     // Chave da API Writekey
const unsigned long channelID = 3140279;          // ID do canal Thingspeak (Thingspeak Channel)

WiFiClient espClient;
PubSubClient client(espClient);

// Configuração de intervalos:
const float TEMP_LIMITE = 28.00;  // <- Temperatura limite
const float TEMP_OFFSET = -2.0;   // <- Ajuste fino temperatura (aferiçao em relação à temperatura real)
...
// NTP para timestamp: (sincronização de hora)
...
void showWelcomeScreen() {
  ...
  // Nas linhas abaixo você insere o nome de seu projeto a ser apeesentado no display:
  drawCenteredText("Seu Projeto", -1, startY, 2, ST77XX_WHITE);            
  drawCenteredText("Seu projeto", -1, startY + 30, 2, ST77XX_CYAN);
  
  int lineY = startY + 70;
  tft.drawFastHLine(SCREEN_WIDTH/4, lineY, SCREEN_WIDTH/2, ST77XX_BLUE);
  
  drawCenteredText("Seu projeto", -1, lineY + 20, 2, ST77XX_YELLOW);
...
void drawNormalScreen(float temp, float umid, bool chovendo, float insolacao) {
  ...
  if (needFullRedraw) {
    ...
    // insira o nome de seu projeto para ser visualizado no display
    drawCenteredText("Seu Projeto", -1, 8, 1, ST77XX_CYAN);
    tft.drawFastHLine(SCREEN_WIDTH/6, 22, 2*SCREEN_WIDTH/3, ST77XX_BLUE);
    
    // Labels fixos
    // Visualização dos tópicos no display
    // Se preferir, indira os nomes de seus tópicos aqui abaixo
...
void loop() {
  ...
  if (now - lastMsg >= MQTT_UPDATE_INTERVAL) {
    ...
    // Publicação dos dados capturados pelos sensores:
    Serial.println("\n=== PUBLICACAO DADOS ===");
    ...
    // Publicação na plataforma Blynk ( Virtual pins: V0, V1, V2, V3 e V4):
    ...
    // Publicação e atualização dos gráficos no Thingspeak:
